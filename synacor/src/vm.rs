...
use crate::parser::{read_binary_file, u16_to_u8_le_bytes, u8_to_u16_le_bytes, write_binary_file};
...
impl Vm {
  ...
  /// Loads little-endian u16 values from `filename` as instructions into memory.
  pub fn load_memory(&mut self, filename: &str) -> Result<usize, VmError> {
    let bytes = read_binary_file(filename)?;
    if bytes.len() / 2 < self.memory.len() {
      u8_to_u16_le_bytes(&bytes, &mut self.memory)?;
    } else {
      let reg_start = self.memory.len() * 2;
      let stk_start = reg_start + self.registers.len() * 2;
      u8_to_u16_le_bytes(&bytes[..reg_start], &mut self.memory)?;
      u8_to_u16_le_bytes(&bytes[reg_start..stk_start], &mut self.registers)?;
      self.stack = vec![0; (bytes.len() - stk_start) / 2];
      u8_to_u16_le_bytes(&bytes[stk_start..], &mut self.stack)?;
    }
    Ok(bytes.len())
  }

  /// Saves memory as little-endian u16 bytes in `filename`.
  pub fn save_memory(&mut self, filename: &str) -> Result<usize, VmError> {
    let mem_bytes = u16_to_u8_le_bytes(&self.memory)?;
    let reg_bytes = u16_to_u8_le_bytes(&self.registers)?;
    let stk_bytes = u16_to_u8_le_bytes(&self.stack)?;
    write_binary_file(&[mem_bytes, reg_bytes, stk_bytes].concat(), filename)
  }
  ...
  pub fn run(
    ...
    loop {
      // if pc == 5489 {
      //     dbg!(pc, self.read(pc), &self.registers);
      //     dbg!(&self.stack);
      // }
      match self.read(pc) {
        // halt: 0
        ...
        20 => {
          ...
          if c == b'*' {
            println!(
              "[Dumping state to core.bin! \npc: {:?}; registers: {:?}; stack: {:?}]",
              pc, self.registers, self.stack
            );
            if self.save_memory("core.bin").is_err() {
...
mod tests {
  ...
  fn executes_simple_program() {
    ...
    vm.run(&mut input, &mut output, Address::Start)
